...
Write-Output "Setting up App Service Plan..."
az appservice plan create --name $Env:APP_SERVICE_PLAN --resource-group $Env:RESOURCE_GROUP --location southcentralus --sku FREE --is-linux

Write-Output "Creating Web App: $Env:WEB_APP_NAME..."
$resp = az webapp create --name $Env:WEB_APP_NAME --plan $Env:APP_SERVICE_PLAN --resource-group $Env:RESOURCE_GROUP --runtime "DOTNETCORE:6.0"
$webApp = -join $resp | ConvertFrom-Json
Write-Output $webApp
...
Write-Output "Done publishing $package to $($webApp.defaultHostName)!"